...
| `smooth-shadow-ring` | Elevated surfaces: `smooth-shadow-ring-*`, never border+shadow | Bundled in `.claude/skills/` |
...
| `smooth-shadow-ring` | **Default when styling elevated surfaces** (card, dialog, popover, dropdown, menu, tooltip, sheet, toast). Prefer `smooth-shadow-ring-*` over `border`/`ring` + `shadow`. |
