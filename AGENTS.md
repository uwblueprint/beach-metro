...
| `smooth-shadow-ring` | Elevated surfaces: `smooth-shadow-ring-*`, never border+shadow | Bundled in `.claude/skills/` (needs shadow-plugin) |
...
| `smooth-shadow-ring` | **Default when styling elevated surfaces** (card, dialog, popover, dropdown, menu, tooltip, sheet, toast) **if shadow-plugin is installed**. Prefer `smooth-shadow-ring-*` over `border`/`ring` + `shadow`. Until the plugin is present, keep using this repo’s existing `shadow-ring-*` tokens. |
